...
int main()
{
    ...
    int serverNum,vmNum,days,req_Num,migrationNum=0; 
    ...
    for (int day = 0; day < days; day++){
        ...
        int migrationMax = floor(vm2server.size()/200.0);        // max migration times
        migrationNum += migrationMax;
        sort(running_servers.begin(), running_servers.end(), cmpRunningServerCoreAndRamMore);
        for (int sIndex = 0; sIndex < runningServersNum; sIndex++){

            if(migrationMax == 0) break;
            // iter src servers
            ...
            int dIndex = runningServersNum-1;
                ...
                if(sid == slen){ // purchase server
                    if(reqs[day][rid].mode == 0){ // single node   --- purchase server
                        for (int i=0; i < choice_servers.size();i++){
                            if(choice_servers[i].a.core >= reqs[day][rid].core && 
                                choice_servers[i].a.ram >= reqs[day][rid].ram){
                                deploy vm = create_vm(reqs[day][rid],1,0);
                                vm2server[vm.id] = id;
                                vm2deploy[vm.id] = vm;
